...
## 3. Storage — READ THIS (the hard part on this cluster)

### The problem chain (all verified on this cluster)
1. Dify's **plugin daemon** writes plugin package files whose names contain a **colon** (`:`),
   e.g. `openai_api_compatible:0.0.66@...`.
2. The cluster's only storage class is **`gl4f-filesystem`** (provisioner `csi.vastdata.com` — VAST/NFS),
   which **rejects filenames containing `:`** (`EINVAL` / "Invalid argument"). There is **no block/ext4
   storage class**. So the plugin daemon cannot use the NFS PVC.
3. **S3 is the documented alternative** (object keys allow colons), but on this cluster it is blocked by TLS:
...
**Production recommendation:** the platform should provide either a correctly-SAN'd VAST S3 certificate
(covering the `...hpecolo.net` access host) or an S3 endpoint under a covered hostname, plus an app-scoped
S3 credential. Until then, Option 2 is the working answer.
...
A chart with declared-but-unvendored deps FAILS at import. Vendor + repackage:
      ...
      tar -xzf dify-0.39.0-pcai2.tgz            # -> ./dify
      helm repo add bitnami  https://charts.bitnami.com/bitnami
      helm repo add weaviate https://weaviate.github.io/weaviate-helm
      helm repo update
      helm dependency update ./dify             # regenerates Chart.lock + charts/  (use 'update', not 'build')
      helm package ./dify --dependency-update    # -> dify-0.39.0-pcai2.tgz WITH charts/ vendored
      tar -tzf dify-0.39.0-pcai2.tgz | grep 'dify/charts/'   # must list postgresql, redis, weaviate

(Or run `finalize-dify-package.sh`.) If reusing a previously vendored `./dify/charts/`, skip the update.
...
6. **Validate = a message returns an answer via Qwen** (UI and/or API). This is the D1 pass.

## 7. Validate via API (D1)
