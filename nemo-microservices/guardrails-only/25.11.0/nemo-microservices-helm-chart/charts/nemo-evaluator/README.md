...
| external.jobs.endpoint | string | `"http://nemo-core-api:8000"` | The external URL of the Jobs microservice. |
...
| external.nimProxy.endpoint_internal | string | `""` | The internal cluster URL of the NeMo NIM Proxy microservice. This is used to remap model URLs from evaluation jobs that match external.nimProxy.endpoint to the internal URL when cluster ingress is not supported for the external address. |
