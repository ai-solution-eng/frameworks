...
#  to set up OIDC for Penpot programmatically (adds the Penpot callback to the shared `ua` client
#  and prints the clientSecret for the Import Framework values override box).

# Define the realm, client, and name of the virtual service for the AIE hosted app
REALM="UA"
CLIENT="ua"
VIRTUAL_SERVICE_NAME="penpot"
...
OIDC_CALLBACK_URL="https://penpot.${DOMAIN_NAME}/api/auth/oidc/callback"
REDIRECT_URIS=$(curl -k -s -X GET "${KEYCLOAK_URL}/admin/realms/${REALM}/clients/${CLIENT_ID}" \
...
curl -k -X PUT "${KEYCLOAK_URL}/admin/realms/${REALM}/clients/${CLIENT_ID}" \
    -H "Content-Type: application/json" \
