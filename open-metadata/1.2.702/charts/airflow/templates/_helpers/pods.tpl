...
- name: check-db
  ...
  args:
    ...
    - "chown -R airflow:root /opt/airflow/logs && chown -R airflow:root /opt/airflow/dags && exec timeout 60s airflow checkdb"
    {{- else }}
    - "chown -R airflow:root /opt/airflow/logs && chown -R airflow:root /opt/airflow/dags && exec timeout 60s airflow db check"
    {{- end }}
  {{- if .volumeMounts }}
  volumeMounts:
